...
{{
    "questions": [
        ...
        try:
            logging.info("Generating questions using gemini-2.0-pro-exp-02-05...")
            response = await self.client.chat.completions.create(
                model="gemini-2.0-pro-exp-02-05", # 不要修改gemini-2.0-pro-exp-02-05，这是正确的模型名字
                messages=[
...
1. 全面覆盖所有评估项目：确保生成的问题能够覆盖HAMD量表的所有核心评估领域。

2. 深度个性化与一般性探索的平衡：仔细分析患者信息，将关键细节自然地融入到相关问题中。
...
4. 问题字段的完整性与质量：每个问题必须包含：
   - id: 问题的唯一标识符
   - question: 问题内容
   - type: 问题类型
   - expected_topics: 预期话题列表
   - follow_up_questions: 追问问题列表

请确保生成的JSON格式正确，所有字段都必须存在。最终的输出必须是一个有效的JSON对象，包含questions数组。"""
