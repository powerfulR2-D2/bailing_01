        ...
        try:
            if self.callback:
                self.callback({
                    ...
                    "need_confirm": response_message.get('need_confirm', False)  # 如果没有need_confirm字段，默认为False
                })
    ...
    async def chat_tool(self, query):
        try:
            ...
            async with self.async_lock:
                ...
                try:
                    # 等待任务完成，设置超时时间
                    llm_responses = await asyncio.wait_for(self.current_task, timeout=60.0)  # 60秒超时
            ...
            return {
                "response": content,
                "need_confirm": llm_responses.get('need_confirm', False),  # 如果没有need_confirm字段，默认为False
